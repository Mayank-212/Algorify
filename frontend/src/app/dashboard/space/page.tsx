...
export default function LearningSpacePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <Card variant="glass" className="p-8 border-dashed border-2 border-border-hover flex flex-col items-center justify-center text-center relative overflow-hidden group bg-bg-glass backdrop-blur-3xl shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
        <div className="absolute inset-0 bg-accent-emerald/5 opacity-0 group-hover:opacity-100 transition-opacity" />
        <div className="w-16 h-16 rounded-full bg-accent-emerald/20 flex items-center justify-center mb-4 border border-accent-emerald/30 shadow-[0_0_20px_rgba(16,185,129,0.2)] group-hover:scale-110 transition-transform">
          <Upload className={`w-8 h-8 text-accent-emerald ${uploading ? "animate-bounce" : ""}`} />
        </div>
        <h3 className="font-bold text-xl mb-2 text-text-primary drop-shadow-sm">Upload a Textbook or Notes</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
        {/* Left: Library */}
        <div className="flex flex-col h-[500px] md:h-[600px] space-y-4">
          <h3 className="font-bold text-lg flex items-center gap-2 shrink-0 text-text-primary drop-shadow-sm">
            <Library className="w-5 h-5 text-accent-emerald" /> Your ML-Indexed Library
          </h3>
          <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
            {books.length === 0 ? (
              <div className="p-8 text-center text-text-muted border border-border-primary rounded-2xl bg-bg-glass backdrop-blur-xl shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]">
              ...
              books.map((book) => (
                <Card 
                  ...
                  className={`p-4 transition-all duration-300 cursor-pointer flex gap-4 border-2 relative overflow-hidden group ${
                    selectedBook === book 
                      ? "bg-accent-emerald/10 border-accent-emerald shadow-[0_0_20px_rgba(16,185,129,0.3)] scale-[1.02]" 
                      : "border-border-primary bg-bg-glass hover:bg-bg-tertiary hover:border-accent-emerald/30 hover:shadow-[0_0_15px_rgba(16,185,129,0.1)] hover:scale-[1.01]"
                  }`}
                >
                  ...
                  <div className={`w-16 h-20 rounded-xl flex items-center justify-center shrink-0 border relative z-10 transition-colors ${selectedBook === book ? "bg-accent-emerald/20 border-accent-emerald/50" : "bg-bg-card border-border-primary group-hover:border-accent-emerald/30"}`}>
                    <BookOpen className={`w-6 h-6 ${selectedBook === book ? "text-accent-emerald drop-shadow-[0_0_8px_rgba(16,185,129,0.8)]" : "text-text-muted group-hover:text-accent-emerald/70"}`} />
                  </div>
        ...
        <div className="flex flex-col h-[500px] md:h-[600px] space-y-4">
          <h3 className="font-bold text-lg flex items-center gap-2 shrink-0 text-text-primary drop-shadow-sm">
            <Bot className="w-5 h-5 text-accent-blue" /> Ask Your Books (RAG)
          </h3>
          <Card variant="glass" className="p-4 flex flex-col flex-1 bg-bg-glass backdrop-blur-3xl border-border-primary shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
            {!selectedBook ? (
              ...
              <div className="flex-1 flex flex-col">
                <div className="mb-4 shrink-0">
                  <span className="text-xs font-bold uppercase tracking-widest text-emerald-400 border border-emerald-400/30 bg-emerald-400/10 px-3 py-1.5 rounded-lg inline-flex items-center gap-2 shadow-[0_0_10px_rgba(16,185,129,0.1)]">
                    <Target className="w-3 h-3" /> Target: {selectedBook}
                  </span>
                </div>
                <div className="flex-1 overflow-y-auto mb-4 border border-border-primary rounded-2xl p-4 sm:p-6 bg-bg-card text-sm custom-scrollbar shadow-inner">
                  {answer ? (
                    <div className="flex gap-3 items-start animate-in fade-in slide-in-from-bottom-2">
                      <div className="w-8 h-8 rounded-lg bg-accent-emerald/20 border border-accent-emerald/50 flex items-center justify-center shrink-0 mt-1">
                        <Bot className="w-4 h-4 text-accent-emerald drop-shadow-[0_0_5px_rgba(16,185,129,0.8)]" />
                      </div>
                     ...
                     <div className="flex flex-col gap-3 items-center text-text-muted justify-center h-full">
                        ...
                        <span className="font-medium animate-pulse">Retrieving facts via AI Engine...</span>
                     </div>
                  ) : (
      ...
      {activeTab === "youtube" && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <Card variant="glass" className="p-6 md:p-8 bg-bg-glass backdrop-blur-3xl border border-border-primary shadow-[0_8px_32px_rgba(0,0,0,0.5)] relative overflow-hidden">
            <div className="absolute -top-[50%] -right-[10%] w-64 h-64 bg-red-500/20 rounded-full blur-[100px] mix-blend-screen pointer-events-none" />
            
            <div className="flex flex-col md:flex-row md:items-center gap-4 mb-6 relative z-10">
              <div className="w-12 h-12 rounded-xl bg-red-500/20 flex items-center justify-center border border-red-500/50 shadow-[0_0_20px_rgba(239,68,68,0.3)] shrink-0">
                <MonitorPlay className="w-6 h-6 text-red-500" />
              </div>
              <div>
                <h3 className="font-bold text-xl text-text-primary drop-shadow-sm">Generate Notes from Video</h3>
            ...
            <div className="flex flex-col sm:flex-row gap-3 relative z-10">
              ...
              <button 
                ...
                className="bg-red-500/20 text-red-500 border border-red-500/50 px-8 py-3.5 rounded-xl font-bold uppercase tracking-widest hover:bg-red-500 hover:text-text-primary transition-all shadow-[0_0_20px_rgba(239,68,68,0.2)] hover:shadow-[0_0_30px_rgba(239,68,68,0.4)] flex items-center justify-center gap-2 disabled:opacity-50 shrink-0"
              >
          ...
          {ytNotes && (
            <div className="space-y-4">
              <div className="flex justify-end">
                <button 
                  onClick={exportPDF}
                  className="flex items-center gap-2 bg-bg-glass border border-border-primary hover:border-emerald-500/50 hover:bg-emerald-500/10 px-5 py-2.5 rounded-xl text-sm font-bold uppercase tracking-widest transition-all text-text-primary hover:text-emerald-400 shadow-[0_4px_20px_rgba(0,0,0,0.3)]"
                >
                  <Download className="w-4 h-4" /> Export PDF
                </button>
              </div>
              <Card variant="glass" className="p-1 md:p-2 bg-bg-glass backdrop-blur-3xl border border-border-primary shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
                <div id="youtube-notes-container" className="prose-chat prose-sm md:prose-base max-w-none text-text-primary bg-bg-card p-6 md:p-10 rounded-[1.25rem] border border-border-primary">
                  <ReactMarkdown 
                    remarkPlugins={[remarkGfm, remarkMath]}
                    rehypePlugins={[rehypeKatex]}
                  >
